...
function extractSessionId(event: StreamEvent): string {
  return (
    event.session_id ||
    String(event.metadata?.session_id || event.metadata?.id || "")
  );
}
 ...
 *   message: 后端学习记录消息。
 * 输出：
 *   返回 web_new 内部能力标识；没有能力时返回 undefined。
 */
function extractMessageCapability(message: SessionMessage): CapabilityName | undefined {
  return message.capability ? fromBackendCapability(message.capability) : undefined;
}
...
/**
 * 管理 web_new 演示聊天状态和 WebSocket 调用。
 *
 * 输入：
 *   initialCapability: 页面入口指定的初始能力。
 * 输出：
 *   返回消息、事件、连接状态、等待状态、能力与工具控制函数。
 */
export function useUnifiedChat(initialCapability: CapabilityName = "chat") {
  ...
  const [tools, setTools] = useState<ToolName[]>(() =>
    defaultToolsForCapability(initialCapability)
  );
   ...
   * 输出：
   *   无；通过 WebSocket 发送 start_turn，并更新本地消息与事件状态。
   */
  function send(
    content: string,
   ...
   * 输出：
   *   返回加载完成的 Promise；同时恢复消息、能力、工具和知识库状态。
   */
  async function loadSession(targetSessionId: string): Promise<void> {
    ...
    const nextKnowledgeBases = detail.preferences?.knowledge_bases || [];
